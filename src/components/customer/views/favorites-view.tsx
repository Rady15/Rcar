...
export function FavoritesView() {
  const { favorites, setSelectedCarId, setCustomerView } = useAppStore();
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-6xl">
      ...
      <p className="text-muted-foreground mb-6">{favorites.length} saved {favorites.length === 1 ? "car" : "cars"}</p>
      {loading ? (
        ...
        <Card className="p-12 text-center"><Heart className="h-12 w-12 text-muted-foreground mx-auto mb-3" /><h3 className="font-semibold mb-1">No favorites yet</h3><p className="text-sm text-muted-foreground mb-4">Tap the heart icon on any car to save it here for later.</p><Button onClick={() => setCustomerView("browse")}><Search className="h-4 w-4 mr-2" />Browse cars</Button></Card>
      ) : (
